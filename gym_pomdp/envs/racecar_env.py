...
class TMazeRacecarGymEnv(gym.Env):
  ...
  def _build_tmaze(self):
      ...
      self.cube_id = self._p.loadURDF("cube_black.urdf", basePosition=[length, -2, 0.5])
      self.cube_id = self._p.loadURDF("cube_black.urdf", basePosition=[length, 2, 0.5])

      self.cube_id = self._p.loadURDF("cube_black.urdf", basePosition=[length-1, -2, 0.5])
      self.cube_id = self._p.loadURDF("cube_black.urdf", basePosition=[length-1, 2, 0.5])

      for i in range(5):
          self.cube_id = self._p.loadURDF("cube_black.urdf", basePosition=[length+1, i-2, 0.5])
  ...
  def _termination(self):
      return False
    # return self._envStepCounter > 250
...
if __name__ == '__main__':
    env = TMazeRacecarGymEnv(isDiscrete=True, renders=True)
